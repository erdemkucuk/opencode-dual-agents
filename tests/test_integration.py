...
def _send_message_to(base_url: str, question: str) -> str:
    """Create a session on the given agent and send a message.
    ...
    with httpx.Client(timeout=60) as client:
        ...
        data = (
            client.post(f"{base_url}/session/{session_id}/message", json=payload)
            .raise_for_status()
            .json()
        )
...
class TestSessionCreation:
    """Scenario 1: POST /session returns a JSON object with an 'id' field."""
...
class TestAgent1Isolation:
    ...
    def test_agent1_responds_to_message(self, agents):
        """Sends a simple prompt directly to Agent 1 and checks the reply.

        Validates: session creation → message → LLM → response.
        """
