#!/usr/bin/env bash
# Run a task on an opencode server asynchronously, poll until complete, and print only the last message.
#
# Identical to opencode_run.sh but prints only the final message instead of the full history.
#
# Usage:
#   ./opencode_run_final.sh <base_url> <prompt> [directory] [timeout_ms]
#
# Example:
#   ./opencode_run_final.sh http://localhost:4098 "List files in /tmp"
...
curl -sf "${BASE}/session/${SESSION_ID}/message${PARAMS}" | jq '.[-1]'
